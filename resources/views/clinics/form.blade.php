<form action="{{route('sedes.store')}}" method="POST">
    ...
    <div class="form-group row">
        ...
        <div class="input-group col-md-6 col-12">
            <input type="text" name="address" value="{{old('address')}}" class="form-control @error('address') is-invalid @enderror" placeholder="Dirección *">
            @error('address')
                <div class="invalid-feedback" role="alert">{{ $message }}</div>
            @enderror
        </div>
    ...
    <div class="form-group row">
        <div class="input-group col-12">
            <textarea class="form-control @error('description') is-invalid @enderror" name="description" placeholder="Descripción" id="" rows="3">{{old('description')}}</textarea>
            @error('description')
                <div class="invalid-feedback" role="alert">{{ $message }}</div>
            @enderror
        </div>
...
@if(Session::has('alert'))
    @component('bootstrap::alert', ['type' => Session::get('alert')->type, 'dismissible' => true , 'animate' => true]) 
